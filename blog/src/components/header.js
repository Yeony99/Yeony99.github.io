    ...
    <div className="wrap flex justify-content-between align-center">
      ...
      <nav className="padding-top-1 margin-0">
        <ul className="flex justify-content-around text-sm margin-0">
          <Link to="/log">
            <ListLink className={"transition duration-500 text-sm margin-1" + (current === "/log" ? " current-path" : " route-path")}>Log</ListLink>
          </Link>
          <Link to="/retrospective">
            <ListLink className="text-sm transition duration-500 margin-1">Retrospective</ListLink>
          </Link>
        </ul>
      </nav>
      {({ path }) => (path)}
